...
<head>
    ...
    <script src="src/javascript/electrodomesticos.js"></script> 
    <style>
...
<body>
    ...
    <div class="container mt-4">
        <h1 class="text-center mb-4">Electrodomésticos de Cocina</h1>
        <div class="row">
            @foreach ($productos as $producto)
                <div class="col-md-4 mb-4">
                    <div class="card" data-toggle="modal" data-target="#productModal" data-id="{{ $producto['id'] }}" data-nombre="{{ $producto['nombre'] }}" data-descripcion="{{ $producto['descripcion'] }}" data-precio="{{ $producto['precio'] }}">
                        <img src="src/imagenes/fotosProductos/{{ $producto['imagen'] }}" class="card-img-top" alt="{{ $producto['nombre'] }}">
                        <div class="card-body">
                            <h5 class="card-title">{{ $producto['nombre'] }}</h5>
                            <p class="card-text">€ {{ $producto['precio'] }}</p>
                            <button class="btn btn-primary add-to-cart" data-id="{{ $producto['id'] }}" data-nombre="{{ $producto['nombre'] }}" data-precio="{{ $producto['precio'] }}">Agregar a la cesta</button>
